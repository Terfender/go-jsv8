...
bool RunExtraCode(v8::Isolate* isolate, v8::Local<v8::Context> context,
  ...
  if (!v8::ScriptCompiler::Compile(context, &source).ToLocal(&script)) return false;
...
ValueTuple v8_Context_Run(ContextPtr ctxptr, const char* code, const char* filename) {
  ...
  const char* origin = filename ? filename : "(no file)";

  v8::Local<v8::String> ogn = v8::String::NewFromUtf8(isolate, origin).ToLocalChecked();
  ...
  v8::MaybeLocal<v8::Value> result = script->Run(ctx->ptr.Get(isolate));
  if (result.IsEmpty()) {
    res.error_msg = DupString(report_exception(isolate, ctx->ptr.Get(isolate), try_catch));

  } else {
      auto x = result.ToLocalChecked();
      res.Value = static_cast<PersistentValuePtr>(new Value(isolate, x));
      res.Kinds = v8_Value_KindsFromLocal(x);
...
void go_callback(const v8::FunctionCallbackInfo<v8::Value>& args) {
  ...
  if (result.error_msg.ptr != nullptr) {
    ...
    args.GetReturnValue().Set(v8::Global<v8::Value>(iso, *static_cast<Value*>(result.Value)));
  }
}
...
PersistentValuePtr v8_Context_Create(ContextPtr ctxptr, ImmediateValue val) {
  ...
  switch (val.Type) {
    ...
    case tDATE: {
      v8::MaybeLocal<v8::Value> maybe_date = v8::Date::New(ctx, val.Float64);
      if (maybe_date.IsEmpty()) {
        return nullptr;
      }
      return new Value(isolate, maybe_date.ToLocalChecked()); break;
      };
...
ValueTuple v8_Value_Get(ContextPtr ctxptr, PersistentValuePtr valueptr, const char* field) {
  ...
  // TODO UTF转码没有check，直接用了ToLocalChecked
...
String v8_Value_String(ContextPtr ctxptr, PersistentValuePtr valueptr) {
  ...
  // TODO 没有check，直接用了ToLocalChecked
